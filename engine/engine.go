...
type EngineMetrics struct { //nolint:revive // user-approved name
	...
	// TransplantResidual{Detached,H2,Pinned,Busy} are GAUGES: the
	// connections a draining engine still held at its last completed sweep
	// cycle, by the reason the hand-off refused them.
	//
	...
	//     sweep keeps running for: a cycle with none of it goes dormant.
	//
	// A standing non-zero Busy after a switch settles is the placement bug
	// itself; Detached, H2 and Pinned are the expected residue.
