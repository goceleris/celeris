...
func TestChainAPISmoke(t *testing.T) {
	s := newServer("chi-chain-smoke", modeH1)
	chainSmoke(t, s)
}

func TestDriverHandlersSkipWithoutServices(t *testing.T) {
	s := newServer("chi-driver-skip", modeH1)
	driverSkipSmoke(t, s)
}

func chainSmoke(t *testing.T, s *Server) {
	t.Helper()
	...
	_ = resp.Body.Close()
	if resp.StatusCode != 200 || len(body) == 0 {
		t.Fatalf("/chain/auth/json status=%d body=%q", resp.StatusCode, body)
	}
...
func driverSkipSmoke(t *testing.T, s *Server) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	ln, err := s.Start(ctx, nil)
